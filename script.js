...
function initOnboarding() {
  ...
  const step5 = document.getElementById('ob-step5');
  ...
  // ── Helper: switch active step by index (0=step1,1=step2,2=step3,3=step4,4=step5) ──
  ...
  document.getElementById('ob-analyze').addEventListener('click', () => {
    ...
    const fixedPct = Number(document.getElementById('sl-fixed').value) || 35;
    const dailyPct = Number(document.getElementById('sl-daily').value) || 20;
    const zakatPct = Number(document.getElementById('sl-zakat').value) || 5;
    const goalMonthly = 0;
    const goalYearly = 0;
    ...
    const payload = {
      salary: sal, fixed: fixedPct, daily: dailyPct,
      entertainment: entPct, savings: savePct, charity: zakatPct
    };


    const fillResults = (health, monthSave, months, pct, recs) => {
      const setText = (id, v) => { const el = document.getElementById(id); if (el) el.textContent = v; };
      const setWidth = (id, w) => { const el = document.getElementById(id); if (el) el.style.width = w; };
      setText('ai-goal-pct', pct + '%');
      setWidth('ai-goal-bar', pct + '%');
      setText('ai-months', months + ' شهراً');
      setText('ai-monthly-save', monthSave.toLocaleString('ar-SA') + ' ر.س');
     

setText('ai-health', health + ' / 100');
      const recText = recs
        ? (recs.find(r => r.includes('ترفيه')) || recs[1] || '')
        : 'حاول تقليص الترفيه بنسبة 5% شهرياً.';
      setText('ai-rec-entertain', recText);
      const motivEl = document.getElementById('ai-motiv-msg');
      if (motivEl) motivEl.textContent = months <= 12
        ? 'رائع! ستحقق هدفك خلال سنة واحدة فقط 🌟'
        : months <= 24
        ? 'أنت على الطريق الصحيح! مدينتك ستنمو قريباً.'
        : 'كل رحلة تبدأ بخطوة. زد نسبة ادخارك وستصل! 💪';
    };

    

      const saveUser = (analysis) => {
        STATE.user = {
          name, email, idNo, bank, salary: sal, savePct, entPct,
          registeredAt: new Date().toISOString(), selectedGoals, analysis
        };
        persistState();
        addDoc(collection(db, 'users'), { ...STATE.user, createdAt: new Date().toISOString() })
          .then(() => console.log('✅ Saved to Firestore'))
          .catch(e => console.warn('⚠️ Firestore:', e.message));
      };
      ...
      fetch('http://127.0.0.1:5000/analyze', {
        ...
        .then(res => res.json())
        .then(result => {
          const monthSave = result.monthly_save || Math.round(sal * savePct / 100);
          const health = result.health_score || Math.min(100, 50 + savePct);
          const target = Math.max(goalYearly || monthSave * 12, 1);
          const months = result.months_to_goal || (monthSave > 0 ? Math.ceil(target / monthSave) : 24);
          const pct = Math.min(100, Math.round((monthSave * 12 / target) * 100));
          fillResults(health, monthSave, months, pct, result.recommendations || null);
          saveUser(result);
          showStep(4);
        })
        .catch(err => {
          ...
          const target = Math.max(goalYearly || monthSave * 12, 1);
          const months = monthSave > 0 ? Math.ceil(target / monthSave) : 24;
          const pct = Math.min(100, Math.round((monthSave * 12 / target) * 100));
          const health = Math.min(100, 50 + savePct + (entPct < 10 ? 10 : 0));
          fillResults(health, monthSave, months, pct, null);
          saveUser({ health_score: health, monthly_save: monthSave, offline: true });
          showStep(4);
        });
      });
      ...
      document.getElementById('ob-enter-app').addEventListener('click', () => {
        ...
        setTimeout(() => {
          overlay.classList.add('ob-hidden');
          chartsInited = false; // force re-init of charts on first dashboard load
    ...
    function initCharts() {
      chartsInited = true;
// ── ملأ بيانات المستخدم الحقيقية ──
   
      const user = STATE.user || {};
      const salUser      = user.salary   || 8500;
      const fixedPct = user.analysis ? (user.analysis.monthly_save ? Math.round(user.analysis.monthly_save / salUser * 100) : 35) : 35;
      const savePct  = user.savePct  || 20;
      const entPct   = user.entPct   || 7;
      const health   = (user.analysis && user.analysis.health_score) || 82;
      const fixedAmtV = Math.round(salUser * fixedPct / 100);
      const dailyPctV = Math.max(100 - fixedPct - savePct - entPct - 5, 0);
      const dailyAmtV = Math.round(salUser * dailyPctV / 100);
      const entAmtV   = Math.round(salUser * entPct / 100);
      const saveAmtV  = Math.round(salUser * savePct / 100);
      const zakatAmtV = Math.round(salUser * 5 / 100);
      const fmtR = n => n.toLocaleString('ar-SA') + ' ر.س';
      const setEl = (id, v) => { const el = document.getElementById(id); if(el) el.textContent = v; };
      setEl('analysis-income',        fmtR(salUser));
      setEl('analysis-fixed',         fmtR(fixedAmtV));
      setEl('analysis-fixed-pct',     fixedPct + '%');
      setEl('analysis-daily',         fmtR(dailyAmtV));
      setEl('analysis-daily-pct',     dailyPctV + '%');
      setEl('analysis-entertain',     fmtR(entAmtV));
      setEl('analysis-entertain-pct', entPct + '%');
      setEl('analysis-save',          fmtR(saveAmtV));
      setEl('analysis-save-pct',      savePct + '%');
      setEl('analysis-zakat',         fmtR(zakatAmtV));
      setEl('analysis-zakat-pct',     '5%');
      drawGauge(health);
      drawDonut(salUser, fixedPct, savePct, entPct);
      drawBar();
    }
    ...
    /* ── Donut chart ── */
    function drawDonut(sal, fixedPct, savePct, entPct) {
      ...
      const dailyPct = Math.max(100 - fixedPct - savePct - entPct - 5, 0);
      const data = [
        { label: 'التزامات', value: fixedPct,  color: '#3DB87A' },
        { label: 'يومية',    value: dailyPct,  color: '#64C8A0' },
        { label: 'ترفيه',   value: entPct,    color: '#F5A623' },
        { label: 'مدخرات',  value: savePct,   color: '#4A90D9' },
      ...
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText(sal.toLocaleString('ar-SA'), cx, cy - 8);
      ctx.fillStyle = '#8FA89A';
